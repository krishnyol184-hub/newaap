        ...
        {tabs.map(t => {
          ...
          return (
            <button key={t.id} onClick={() => setTab(t.id)}
              className={`flex items-center gap-2 px-4 py-2.5 rounded-xl font-semibold text-sm whitespace-nowrap transition ${tab === t.id ? "bg-teal-500/15 text-teal-300 border border-teal-500/30" : "text-slate-400 hover:text-white"}`}
        ...
        <div className="space-y-6" data-testid="admin-overview">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              { k: "solar_apps", label: lang === "hi" ? "सोलर आवेदन" : "Solar Applications", icon: FaSolarPanel, color: "from-teal-500 to-teal-700" },
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div className="glass p-5">
              <h3 className="font-semibold text-white mb-3 flex items-center gap-2"><FaSolarPanel className="text-teal-400" /> {lang === "hi" ? "सोलर स्थिति" : "Solar Status"}</h3>
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between"><span className="text-slate-400 flex items-center gap-2"><FaCheckCircle className="text-teal-400" /> {lang === "hi" ? "स्वीकृत" : "Approved"}</span><b className="text-teal-300">{stats.solar_approved}</b></div>
              </div>
            </div>
            <div className="glass p-5">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between"><span className="text-slate-400 flex items-center gap-2"><FaCheckCircle className="text-teal-400" /> {lang === "hi" ? "स्वीकृत" : "Approved"}</span><b className="text-teal-300">{stats.loan_approved}</b></div>
              </div>
            </div>
          </div>
        </div>
              ...
              <tbody>
                {(tab === "solar" ? solar : loan).map(a => (
                  <tr key={a.id} data-testid={`admin-${tab}-row-${a.ref_no}`}>
                    <td className="font-mono font-bold text-teal-400">{a.ref_no}</td>
            ...
            <div key={i} className="glass p-4" data-testid={`contact-item-${i}`}>
              <div className="flex items-start justify-between gap-3 mb-2">
                <div>
                  ...
                  <div className="text-sm text-teal-400 font-medium mt-0.5">{c.subject}</div>
                </div>
      ...
      {tab === "notices" && (
        <div className="space-y-6" data-testid="admin-notices-cms">
          <form onSubmit={addNotice} className="glass p-5" data-testid="notice-add-form">
            <h3 className="font-semibold text-white mb-3 flex items-center gap-2"><FaPlus className="text-teal-400" /> {lang === "hi" ? "नई सूचना जोड़ें" : "Add New Notice"}</h3>
