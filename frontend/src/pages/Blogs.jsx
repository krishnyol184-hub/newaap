        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5 mt-8" data-testid="blogs-grid">
          {items.map((b, i) => (
            <Link
              key={b.id}
              to={`/blogs/${b.slug}`}
              className="glass overflow-hidden group hover:border-teal-500/40 transition flex flex-col"
              data-testid={`blog-card-${b.slug}`}
            >
              {b.image_url ? (
                <img src={`${BACKEND}${b.image_url}`} alt={b.title} className="w-full h-44 object-cover" loading="lazy" />
              ) : (
                <div className="w-full h-44 bg-gradient-to-br from-teal-900/60 to-slate-900 grid place-items-center">
                  <FaNewspaper className="text-4xl text-teal-500/40" />
                </div>
              )}
              <div className="p-5 flex flex-col flex-1">
                ...
                <h2 className="font-display font-bold text-white text-lg leading-snug group-hover:text-teal-400 transition mb-2">{b.title}</h2>
                <p className="text-sm text-slate-400 line-clamp-2 flex-1">{b.excerpt}</p>
                <span className="mt-3 text-sm font-semibold text-teal-400 inline-flex items-center gap-1">
