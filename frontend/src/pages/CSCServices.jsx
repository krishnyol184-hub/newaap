    ...
    <div className="max-w-7xl mx-auto px-4 py-10" data-testid="csc-page">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6" data-testid="csc-trust-badges">
        <div className="glass p-4 flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-teal-500/10 border border-teal-500/30 text-teal-400 flex items-center justify-center"><FaShieldAlt /></div>
          <div>
      ...
      <div className="glass p-4 mb-6 flex flex-col md:flex-row gap-3 items-stretch md:items-center" data-testid="csc-filter-bar">
        ...
        <div className="flex gap-2 overflow-x-auto">
          <button onClick={() => setActiveCat("all")}
            className={`px-3 py-2 rounded-lg text-xs font-semibold whitespace-nowrap ${activeCat === "all" ? "bg-teal-500/15 text-teal-300 border border-teal-500/30" : "bg-white/[0.03] text-slate-400 border border-white/10 hover:text-white"}`}
            data-testid="csc-cat-all">
            {lang === "hi" ? "सभी" : "All"} ({totalCount})
          </button>
          {CSC_CATEGORIES.map(cat => (
            <button key={cat.id} onClick={() => setActiveCat(cat.id)}
              className={`px-3 py-2 rounded-lg text-xs font-semibold whitespace-nowrap ${activeCat === cat.id ? "bg-teal-500/15 text-teal-300 border border-teal-500/30" : "bg-white/[0.03] text-slate-400 border border-white/10 hover:text-white"}`}
            ...
            return (
              <div key={cat.id} data-testid={`csc-cat-block-${cat.id}`}>
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-9 h-9 rounded-xl bg-teal-500/10 border border-teal-500/30 text-teal-400 flex items-center justify-center"><Icon /></div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  {cat.services.map(s => (
                    <Link key={s.id} to={`/csc/apply?service=${s.id}`}
                      className="glass p-4 flex items-center justify-between hover:border-teal-500/40 transition group"
                      ...
                      <FaChevronRight className="text-slate-500 group-hover:text-teal-400 group-hover:translate-x-1 transition shrink-0" />
                    </Link>
                  ))}
                </div>
              </div>
