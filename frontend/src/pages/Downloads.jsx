    ...
    <div className="max-w-5xl mx-auto px-4 py-12" data-testid="downloads-page">
      ...
      {templates.length === 0 ? (
        ...
        <div className="grid md:grid-cols-2 gap-4 mt-4" data-testid="cv-templates-grid">
          {templates.map((t) => (
            <a key={t.id} href={`${BACKEND}${t.url}`} target="_blank" rel="noreferrer" className="glass p-5 flex items-center gap-4 hover:border-teal-500/40 transition" data-testid={`cv-template-${t.id}`}>
              <FaFileAlt className="text-teal-400 text-3xl shrink-0" />
              <div className="flex-1">
                <div className="font-semibold text-white">{t.name}</div>
                <div className="text-xs text-slate-400 mt-1">{t.filename}{t.size ? ` · ${(t.size / 1024).toFixed(0)} KB` : ""}</div>
              </div>
              <FaFileDownload className="text-teal-400 text-xl" />
            </a>
          ))}
        </div>
      )}
      ...
      <div className="grid md:grid-cols-2 gap-4 mt-4">
        {items.map((d, i) => (
          <a key={d.id} href={d.url} className="glass p-5 flex items-center gap-4 hover:border-teal-500/40 transition" data-testid={`download-item-${i}`}>
