      ...
      <div className="mt-8 space-y-3">
        {items.map((f, i) => (
          <div key={f.id} className="glass overflow-hidden" data-testid={`faq-item-${i}`}>
            <button className="w-full flex items-center justify-between p-4 text-left font-semibold text-white hover:bg-teal-500/5 transition"
              onClick={() => setOpen(open === i ? -1 : i)} data-testid={`faq-toggle-${i}`}>
              <span className="flex items-center gap-2"><FaQuestionCircle className="text-teal-400" /> {lang === "hi" ? f.q_hi : f.q_en}</span>
              <FaChevronDown className={`transition-transform text-teal-400 ${open === i ? "rotate-180" : ""}`} />
            </button>
