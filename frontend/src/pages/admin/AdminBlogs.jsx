...
const inputCls =
  "w-full px-3 py-2 rounded border border-slate-300 focus:border-teal-500 focus:ring-2 focus:ring-teal-500/20 outline-none text-sm text-slate-900 bg-white";
const EMPTY = {
    ...
    return (
      <div data-testid="admin-blogs-page">
        <div className="flex flex-wrap items-center gap-3 mb-5">
          ...
          <button onClick={openCreate} className="px-3 py-1 rounded border border-teal-600 text-teal-700 hover:bg-teal-50 text-[13px] font-medium inline-flex items-center gap-1.5" data-testid="admin-add-blog-btn">
        ...
        <div className="bg-white rounded border border-slate-200 shadow-sm" data-testid="admin-blogs-list">
          ...
          {loading ? (
            ...
            items.map((b) => (
              <div key={b.id} className="grid grid-cols-1 sm:grid-cols-[1fr_160px_120px_120px] gap-2 items-center px-4 py-3 border-b border-slate-100 hover:bg-teal-50/40 group" data-testid={`admin-blog-row-${b.id}`}>
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className="font-semibold text-teal-800 truncate">{b.title}</p>
                ...
                <div className="flex items-center gap-1 sm:justify-end">
                  <a href={`/blogs/${b.slug}`} target="_blank" rel="noreferrer" className="text-xs font-semibold text-teal-700 hover:underline px-2">View</a>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_290px] gap-5 items-start">
        ...
        <div className="space-y-4 min-w-0">
          <input
            ...
            className="w-full px-4 py-3 rounded border border-slate-300 bg-white text-xl text-slate-900 focus:border-teal-500 focus:ring-2 focus:ring-teal-500/20 outline-none"
          ...
          <div className="bg-white rounded border border-slate-200 shadow-sm" data-testid="rankmath-seo-box">
            <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200">
              <span className="font-semibold text-slate-800 flex items-center gap-2"><FaCog className="text-teal-600" /> HR SEO</span>
            ...
            <div className="flex border-b border-slate-200 text-[13px]">
              {seoTabs.map((t) => (
                <button key={t.id} onClick={() => setSeoTab(t.id)}
                  className={`flex items-center gap-1.5 px-4 py-2.5 border-b-2 -mb-px ${seoTab === t.id ? "border-teal-600 text-teal-700 font-semibold" : "border-transparent text-slate-500 hover:text-slate-700"}`}
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded border border-slate-200 shadow-sm" data-testid="publish-box">
            ...
            <div className="p-4 space-y-3">
              ...
              <div className="flex items-center gap-2 pt-1">
                ...
                <button type="button" disabled={busy} onClick={() => save()} className="flex-1 px-3 py-2 rounded bg-teal-600 hover:bg-teal-700 text-white text-sm font-semibold inline-flex items-center justify-center gap-1.5 disabled:opacity-60" data-testid="admin-blog-save">
          ...
          <div className="bg-white rounded border border-slate-200 shadow-sm" data-testid="categories-box">
            <div className="px-4 py-2.5 border-b border-slate-200 font-semibold text-slate-800 text-sm flex items-center gap-2"><FaFolder className="text-teal-600" /> Categories</div>
            <div className="p-4">
              <div className="max-h-40 overflow-y-auto space-y-1.5 mb-3">
                ...
                {[...new Set([...taxonomy.categories, ...form.categories])].map((c) => (
                  <label key={c} className="flex items-center gap-2 text-[13px] text-slate-700 cursor-pointer">
                    <input type="checkbox" checked={form.categories.includes(c)} onChange={() => toggleCat(c)} className="accent-teal-600" data-testid={`cat-${c}`} /> {c}
                  </label>
                ))}
              </div>
              <div className="flex gap-2">
                ...
                <button type="button" onClick={addCat} className="px-3 rounded border border-teal-600 text-teal-700 text-[13px] font-semibold hover:bg-teal-50" data-testid="add-category-btn">Add</button>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded border border-slate-200 shadow-sm" data-testid="tags-box">
            <div className="px-4 py-2.5 border-b border-slate-200 font-semibold text-slate-800 text-sm flex items-center gap-2"><FaTags className="text-teal-600" /> Tags</div>
            <div className="p-4">
              <div className="flex gap-2 mb-2">
                ...
                <button type="button" onClick={() => addTag()} className="px-3 rounded border border-teal-600 text-teal-700 text-[13px] font-semibold hover:bg-teal-50" data-testid="add-tag-btn">Add</button>
              </div>
              <div className="flex flex-wrap gap-1.5">
                {form.tags.map((t) => (
                  <span key={t} className="inline-flex items-center gap-1 text-[12px] bg-teal-50 text-teal-700 border border-teal-200 rounded-full px-2 py-0.5" data-testid={`tag-chip-${t}`}>
                ...
                <div className="mt-3">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {taxonomy.tags.slice(0, 12).filter((t) => !form.tags.includes(t)).map((t) => (
                      <button key={t} type="button" onClick={() => addTag(t)} className="text-[12px] text-slate-500 border border-slate-200 rounded-full px-2 py-0.5 hover:border-teal-300 hover:text-teal-700">{t}</button>
                    ))}
                  </div>
                </div>
              )}
          ...
          <div className="bg-white rounded border border-slate-200 shadow-sm" data-testid="featured-image-box">
            <div className="px-4 py-2.5 border-b border-slate-200 font-semibold text-slate-800 text-sm flex items-center gap-2"><FaImage className="text-teal-600" /> Featured image</div>
            <div className="p-4">
              {imgPreview ? (
              ...
              <input type="file" accept="image/*" onChange={(e) => pickImage(e.target.files?.[0] || null)}
                className="w-full text-sm text-slate-500 file:mr-3 file:py-2 file:px-4 file:rounded file:border-0 file:text-xs file:font-bold file:bg-teal-50 file:text-teal-700 hover:file:bg-teal-100"
