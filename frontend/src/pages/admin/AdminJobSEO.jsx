    ...
    <div data-testid="admin-job-seo-page">
      <div className="flex flex-wrap items-start justify-between gap-3 mb-6">
        ...
        <button
          onClick={shuffle}
          disabled={shuffling}
          className="px-4 py-2.5 rounded-lg bg-blue-500 hover:bg-blue-600 disabled:opacity-60 text-white text-sm font-semibold inline-flex items-center gap-2 shadow"
                ...
                {data.items.map((v) => (
                  <tr key={v.id} className="hover:bg-slate-50/60" data-testid={`admin-job-seo-row-${v.id}`}>
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      <span className={`text-xs font-bold px-2 py-1 rounded-full ${v.source === "manual" ? "text-blue-700 bg-blue-50" : "text-blue-700 bg-blue-50"}`}>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-5 space-y-4">
              <div className="flex items-center gap-3">
                <span className={`text-2xl font-extrabold ${seo.score >= 75 ? "text-green-600" : seo.score >= 45 ? "text-blue-600" : "text-red-500"}`} data-testid="admin-job-seo-score">
                  {seo.score}/100
                </span>
