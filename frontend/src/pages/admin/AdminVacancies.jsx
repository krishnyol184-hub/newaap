    ...
    <div data-testid="admin-vacancies-page">
      <div className="flex items-start justify-between gap-4 mb-6 flex-wrap">
        <div>
          ...
          <p className="text-sm text-slate-600 max-w-2xl">
            ...
            public <Link to="/" className="text-teal-700 underline">homepage</Link>.
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => setEditing({})}
            className="px-4 py-2.5 rounded-lg bg-teal-600 hover:bg-teal-700 text-white text-sm font-semibold inline-flex items-center gap-2 shadow"
      ...
      {!loading && items.length === 0 && (
        <div className="rounded-2xl border-2 border-dashed border-slate-300 bg-white p-10 text-center">
          ...
          <button
            onClick={() => setEditing({})}
            className="px-4 py-2 rounded-lg bg-teal-600 hover:bg-teal-700 text-white text-xs font-semibold inline-flex items-center gap-2"
              ...
              {items.map((v) => (
                <tr key={v.id} className="border-t border-slate-100 hover:bg-slate-50" data-testid={`admin-vac-row-${v.id}`}>
                  <td className="px-4 py-3 font-medium text-slate-900 max-w-md">
                    <Link to={`/vacancies/${v.id}`} target="_blank" className="hover:text-teal-700 inline-flex items-center gap-1.5">
                  ...
                  <td className="px-4 py-3 hidden lg:table-cell">
                    {v.application_mode ? (
                      <span className={`inline-block px-2 py-0.5 rounded-full text-[10px] font-semibold capitalize ${
                        v.application_mode === "online" ? "bg-teal-100 text-teal-800" : "bg-teal-100 text-teal-800"
